...
for car, info in car_data.items():
    if info[1] >= search_criteria[0] and info[2] >= search_criteria[1] and info[4] <= search_criteria[2]:
        search_result.insert(i, car)
        i += 1

# Sort matching cars by price ascending
search_result.sort(key=lambda car: car_data[car][4])

# Print up to five found cars
print("Top 5 matching cars (sorted by price ascending):")
for car in search_result[:5]:
    print(car, car_data[car])
